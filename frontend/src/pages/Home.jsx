...
function Home({ search }) {
    ...
    const loadLatestMovies = async () => {
      try {
        const res = await axios.get(
          `${API_URL}/api/movies?page=1&limit=100&category=${encodeURIComponent(
            "หนังใหม่ล่าสุด"
          )}`
        );

        const latest = (res.data.movies || []).filter(
          (movie) => Number(movie.year) === 2026
        );
  ...
  const currentMovies = useMemo(() => {
    return movies.filter((movie) => {
      ...
      const matchCategory =
        category === "หนังทั้งหมด" || movie.category?.includes(category);
    ...
    <div>
      ...
      {latestMovies.length > 0 && (
        <section style={styles.carouselSection}>
          <h2 style={styles.sectionTitle}>หนังใหม่ล่าสุด (2026)</h2>
      ...
      <div style={styles.wrapper}>
        <div style={styles.layout}>
          {/* SIDEBAR */}
          <aside style={styles.sidebar} className="hide-mobile">
            <h3 style={styles.sidebarTitle}>หมวดหมู่</h3>
          ...
          <main style={styles.main}>
            <div style={styles.movieTitle}>{category}</div>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <div style={styles.emptyState}>
                    <div style={styles.emptyBox}>
                      ...
                      <div style={styles.emptyText}>กำลังทำการอัพเดท</div>
...
function ArticleSection({ articles }) {
  ...
  return (
    <div style={styles.articleWrapper}>
      <h2 style={styles.articleTitle}>เว็บดูหนังออนไลน์ หนังใหม่ชนโรง 2026</h2>
...
//       const title = movie.title?.toLowerCase() || "";
//       const matchSearch = title.includes(keyword);
//       const matchCategory = category === "หนังทั้งหมด" || movie.category?.includes(category);
...
//           <h2
//             style={styles.sectionTitle} > หนังใหม่ล่าสุด ({LATEST_YEAR})
//           </h2>
...
//                   style={{
//                     ...styles.categoryItem,
//                     color: isActive ? "#ffd000" : "#aaa",
//                     fontWeight: isActive ? "bold" : "normal",
//                     background: isActive ? "rgba(255,208,0,0.12)" : "transparent",
//                     paddingLeft: isActive ? "18px" : "10px",
//                   }}
//                   onClick={() => changeCategory(cat)} > {cat}
//                 </div>);
//             })}
//           </aside>
//           {/* =============================
...
//                 <div style={styles.loadingText} > กำลังโหลดหนัง... </div>
//               </div>
//             ) : (
//               <div className="movie-grid">
//                 {currentMovies.length > 0 ? (
...
//                         <div style={styles.emptyIcon} > 🎬 </div>
//                         <div style={styles.emptyText} > กำลังทำการอัพเดท </div>
//                         <div style={styles.emptySubText} > หมวดหมู่นี้กำลังเพิ่มหนังใหม่ โปรดกลับมาอีกครั้ง </div> </div>
//                     </div>
//                   </div>
//                 )}
//               </div>
...
//     return null;
//   }
//   return (<div style={styles.articleWrapper} >
